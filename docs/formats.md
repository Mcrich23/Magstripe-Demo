...
Detection is a structural heuristic, not verification that the input is a genuine payment card. Duplicate or contradictory tracks are flagged; incomplete tracks are not decoded. An unsupported track does not prevent interpretation of other complete tracks. All unparsed data is masked by default.
