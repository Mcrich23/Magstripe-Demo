...
test('hands-free lifecycle: successive swipes, focus recovery, and automatic clearing', async t => {
  ...
  now += 30_000; tick();
  assert.equal(node('result-card').hidden, true);
  type(samples.membership + ';\n'); finish();
  assert.equal(node('fields').children.length, 3);
  assert.equal(number(), '7000000012345678');
  assert.equal(node('fields').children[2].children[1].textContent, '000000000000');
  assert.equal(node('result-title').textContent, 'Membership card · what the reader sees');
  assert.equal(node('format-note').hidden, true);
  assert.equal(node('membership-markers').hidden, false);
  assert.equal(node('payment-markers').hidden, true);
  assert.equal(node('checks').children.length, 1);
  assert.match(node('read-note').textContent, /Track 2 is incomplete/);
  type(samples.membershipBoth.replace('?;', '?\n;') + '\n'); finish();
  assert.equal(node('fields').children.length, 4);
  assert.equal(node('track-structures').children.length, 2);
  assert.equal(node('fields').children[2].children[0].textContent, 'Track 1 data');
  assert.equal(node('fields').children[2].children[1].textContent, '000000000000');
  assert.equal(node('fields').children[3].children[0].textContent, 'Track 2 data');
  assert.equal(node('fields').children[3].children[1].textContent, '24127990000000000000');
  assert.equal(node('checks').children.length, 1);
  assert.equal(node('checks').children[0].children[1].textContent, 'Match');
  assert.equal(node('read-note').hidden, true);
  assert.equal(node('read-note').textContent, '');
  type(samples.membershipBoth.match(/[^?]+\?/g).reverse().join('\n')); finish();
  assert.equal(node('fields').children[2].children[1].textContent, '000000000000');
  assert.equal(node('fields').children[3].children[1].textContent, '24127990000000000000');
  node('clear-swipe').dispatchEvent(new Event('click'));
  assert.equal(node('fields').children.length, 0);
  type('%<img src=x onerror=alert(1)>?'); finish();
  assert.equal(node('fields-table').hidden, true);
  assert.equal(node('checks-footer').hidden, true);
  assert.equal(node('reading-guide').hidden, true);
  assert.equal(node('markers').hidden, true);
  assert.equal(node('format-note').hidden, true);
  const unknownTrack = node('track-structures').children[0];
  assert.equal(unknownTrack.children[0].children[0].textContent, 'Track 1 · Unrecognized layout');
  assert.equal(unknownTrack.children[1].children[0].textContent, '%<img src=x onerror=alert(1)>?');
  type('\n');
  assert.equal(node('track-structures').children.length, 0);
  type(samples.payment); finish();
  assert.equal(node('checks-footer').hidden, false);
  assert.equal(node('reading-guide').hidden, false);
  assert.equal(node('markers').hidden, false);
  assert.equal(node('fields').children.length, 5);
  assert.equal(node('payment-markers').hidden, false);
  assert.equal(node('membership-markers').hidden, true);
  assert.equal(node('format-note').hidden, false);
